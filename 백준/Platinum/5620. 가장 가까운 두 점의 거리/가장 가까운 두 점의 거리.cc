...
int divide_and_conquer(int start, int end){

	if (start == end) return MAX_DIST;

	int min_dist = dist(v[start], v[end]);

	int mid = (start + end) / 2;
	int left_min = divide_and_conquer(start, mid), right_min = divide_and_conquer(mid + 1, end);
...
int main(){
	...
	printf("%d", divide_and_conquer(0, N - 1));
